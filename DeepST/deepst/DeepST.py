...
class run():
	...
	def _get_adata(
		...
		assert self.platform in ['Visium', 'ST', 'MERFISH', 'slideSeq', 'stereoSeq', 'benchmark_test']
		if self.platform in ['Visium', 'ST']:
			if self.platform == 'Visium':
				# adata = read_10X_Visium(os.path.join(data_path, data_name), count_file=data_name+'_filtered_feature_bc_matrix.h5')
				if data_name in ['151507', '151508', '151509', '151510', '151669', '151670', '151671', '151672', '151673', '151674', '151675', '151676']:
					...
					adata = load_BC(root_dir=data_path, section_id=data_name)
					adata = DLPFC_extra_info(adata)
					save_path_image_crop = Path(os.path.join(self.save_path, 'Image_crop', f'{data_name}'))
					save_path_image_crop.mkdir(parents=True, exist_ok=True)
					adata = image_crop(adata, save_path=save_path_image_crop)
		...
		elif self.platform == 'benchmark_test':
			if data_name in ['STARmap_20180505_BY3_1k.h5ad']:
				...
				adata = load_mHypothalamus(root_dir=data_path, section_id=data_name)
				print(data_name)
				print(adata.obs)
				adata = other_extra_info(adata)
